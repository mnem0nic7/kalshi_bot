...
def _crypto_touch_policy_context(
    ...
    *,
    ...
    btc_15m_touch20_rules_policy: bool = False,
) -> dict[str, Any]:
    ...
    if btc_15m_touch20_rules_policy:
        target_pct = Decimal(str(settings.crypto_btc15m_touch20_take_profit_pct))
        stop_pct = Decimal("1.0000")
        min_market_age_seconds = max(0, int(settings.crypto_btc15m_touch20_min_market_age_seconds))
        min_seconds_to_close = max(0, int(settings.crypto_btc15m_touch20_min_seconds_to_close))
        policy_name = "btc15m_touch20_rules"
        no_initial_hard_stop = True
    elif btc_1h_touch_policy:
...
def _crypto_touch20_rules_artifact_type(
    base: str,
    *,
    frequency: str,
    ...
    symbols = normalize_asset_symbols(asset_symbols)
    if len(symbols) == 1:
        return f"{base}:{freq}:{symbols[0]}"
    return f"{base}:{freq}"


def _crypto_objective_is_touch20(value: str | None) -> bool:
    normalized = str(value or "settlement").strip().lower().replace("_", "-")
    return normalized in {"touch20", "touch-20", "touch-20pct", "touch20-before-close"}


def _crypto_objective_is_touch20_rules(value: str | None) -> bool:
    normalized = str(value or "settlement").strip().lower().replace("_", "-")
    return normalized in {
        "touch20-rules",
        "touch-20-rules",
        "touch20-rules-before-close",
        "touch20-rules-btc15m",
        "btc15m-touch20-rules",
    }
...
def _crypto_touch20_rules_replay_gate_note_updates(
    *,
    frequency: str,
    asset_symbols: list[str] | None,
    status: str,
    ...
    updates: dict[str, dict[str, Any]] = {f"crypto_replay_gate_touch20_rules:{freq}": dict(note)}
    if symbols:
        updates[f"crypto_replay_gate_touch20_rules:{freq}:{','.join(symbols)}"] = dict(note)
    return updates
...
class CryptoReplayService:
    ...
    async def run(
        ...
        *,
        ...
        if _crypto_objective_is_touch20_rules(objective):
            report = await self._build_touch20_rules_report(
                frequency=frequency,
                days=days,
                limit=limit,
                command="run",
            ...
            if persist:
                async with self.session_factory() as session:
                    ...
                    artifact = await repo.record_crypto_model_artifact(
                        ...
                        artifact_type=_crypto_touch20_rules_artifact_type(
                            "backtest_touch20_rules",
                            frequency=report["frequency"],
                            asset_symbols=report.get("asset_symbols") or [],
                        ),
                        version=_version(f"crypto-{report['frequency']}-touch20-rules-backtest", report),
    ...
    async def validate(
        ...
        *,
        ...
        if _crypto_objective_is_touch20_rules(objective):
            return await self._build_touch20_rules_report(
                frequency=frequency,
                days=days,
                limit=limit,
                command="validate",
    ...
    async def gate(
        ...
        *,
        ...
        requested_assets = normalize_asset_symbols(asset_symbols)
        if _crypto_objective_is_touch20_rules(objective):
            touch_assets = [asset for asset in (requested_assets or ["BTC"]) if asset == "BTC"] or ["BTC"]
            async with self.session_factory() as session:
                repo = PlatformRepository(session)
                backtest = await repo.get_latest_crypto_model_artifact(
                    frequency=freq,
                ...
                reasons = _crypto_touch20_rules_replay_gate_reasons(metrics, settings=self.settings)
                gate = {
                    "passed": not reasons,
                    "reasons": reasons,
                    "requirements": _crypto_touch20_rules_replay_requirements(self.settings),
                    "objective": "touch_20pct_before_close",
                    "strategy": "btc15m_touch20_rules",
                    "uses_trained_model": False,
                }
                artifact = await repo.record_crypto_model_artifact(
                    frequency=freq,
                    artifact_type=_crypto_touch20_rules_artifact_type(
                        "replay_gate_touch20_rules",
                        frequency=freq,
                        asset_symbols=touch_assets,
                    ),
                    version=_version(f"crypto-{freq}-touch20-rules-gate", gate),
                ...
                notes.update(
                    _crypto_touch20_rules_replay_gate_note_updates(
                        frequency=freq,
                        asset_symbols=touch_assets,
    ...
    async def _build_touch20_rules_report(
        self,
        *,
        frequency: str,
        days: int | None,
        ...
        spot_rows = _filter_crypto_snapshot_rows(spot_rows, requested_assets)
        snapshots = _filter_snapshots_by_per_asset_funding_cutoff(funding_rate_rows, snapshots)
        rebuilt_rows = _crypto_decision_rows(
            snapshots,
            candles,
            spot_rows,
            funding_rate_rows=funding_rate_rows,
        ...
        if limit and limit > 0:
            rows = rows[-limit:]
        replay = _evaluate_crypto_touch20_rules_replay(
            rows,
            settings=self.settings,
            crypto_policy=crypto_policy,
        )
        ...
        if any(issue["severity"] == "fail" for issue in issues):
            ...
            status = "warn"
        return {
            "schema_version": "crypto-touch20-rules-backtest-report-v1",
            "status": status,
            "command": command,
            "kalshi_env": self.settings.kalshi_env,
            "frequency": freq,
...
class CryptoNonModelTouch20Service:
    ...
    async def run_once(self, *, frequency: str = "15m", asset_symbol: str = "BTC") -> dict[str, Any]:
        freq = normalize_frequency(frequency) or "15m"
        asset = normalize_asset_symbol(asset_symbol)
        if freq != "15m" or asset != "BTC":
            return {
                "status": "unsupported_scope",
                "strategy": BTC15M_TOUCH20_RULES_STRATEGY,
                "frequency": freq,
                "asset_symbol": asset,
                "reason": "btc15m_touch20_rules is scoped to BTC 15m only",
            }
        ...
        async with self.session_factory() as session:
            ...
            funding_rate_rows = await repo.list_crypto_funding_rates_bulk(asset_symbols=[asset])
            ledger = _btc15m_touch20_rules_ledger_payload(
                await repo.get_checkpoint(_btc15m_touch20_rules_ledger_stream(self.settings.kalshi_env, asset, freq)),
                kalshi_env=self.settings.kalshi_env,
                asset_symbol=asset,
                frequency=freq,
            )
        ...
        if not _crypto_touch_replay_gate_passed(gate):
            return {
                "status": "gate_blocked",
                "strategy": BTC15M_TOUCH20_RULES_STRATEGY,
                "kalshi_env": self.settings.kalshi_env,
                ...
                "asset_symbol": asset,
                "gate": gate_summary,
                "reason": "replay_gate_touch20_rules missing or blocked",
            }

        daily_pnl = _btc15m_touch20_rules_daily_realized_pnl(ledger, now)
        ...
        allowed_bucket_keys = set(str(key) for key in (gate_metrics.get("allowed_bucket_keys") or []))
        bucket_by_key = _crypto_bucket_matrix_by_key(gate_metrics.get("bucket_matrix") or [])
        candidates: list[dict[str, Any]] = []
        stale_cutoff = max(30, int(self.settings.crypto_take_profit_stale_snapshot_seconds))
        for snapshot in snapshots:
            funnel["market_seen"] += 1
            observed_at = snapshot.observed_at
            if observed_at.tzinfo is None:
                observed_at = observed_at.replace(tzinfo=UTC)
            if now - observed_at > timedelta(seconds=stale_cutoff):
                skipped.append({"market_ticker": snapshot.market_ticker, "reason": "stale_quote_snapshot"})
                continue
            market = _market_from_snapshot(snapshot)
            if market.status and market.status not in {"open", "active"}:
                skipped.append({"market_ticker": market.market_ticker, "reason": "market_not_open"})
                continue
            if any(value is None for value in (market.yes_bid_dollars, market.yes_ask_dollars, market.no_bid_dollars, market.no_ask_dollars)):
                skipped.append({"market_ticker": market.market_ticker, "reason": "missing_real_bid_ask"})
                continue
            funnel["quote_valid"] += 1
            row = _crypto_live_market_row(
                market,
                spot_rows=spot_rows,
                funding_rate_rows=funding_rate_rows,
                settings=self.settings,
            )
            row_candidates = _crypto_touch_strategy_candidates(
                row,
                settings=self.settings,
                crypto_policy=crypto_policy,
            ...
            if best.get("reason") in {"spot_data_stale", "spot_data_missing_or_stale", "spot_source_proxy_only"}:
                skipped.append({"market_ticker": market.market_ticker, "reason": best.get("reason")})
                continue
            if best.get("reason") != "missing_quote":
                funnel["spot_fresh"] += 1
            selected = next((candidate for candidate in row_candidates if candidate.get("candidate_status") == CRYPTO_LIVE_QUALITY), None)
            if selected is None:
                skipped.append(
                    {
                        "market_ticker": market.market_ticker,
                        "reason": best.get("reason") or "no_live_candidate",
                        "candidate_status": best.get("candidate_status"),
                    }
                )
                continue
            if selected.get("reason") != "touch_spread_above_tier_max":
                funnel["spread_pass"] += 1
            if selected.get("live_entry_window_reason") is None:
                funnel["entry_window_pass"] += 1
            bucket_key = str(selected.get("bucket_key") or "")
            bucket = bucket_by_key.get(bucket_key)
            if bucket_key not in allowed_bucket_keys:
                skipped.append(
                    {
                        "market_ticker": market.market_ticker,
        ...
        candidates.sort(key=_btc15m_touch20_rules_rank_key, reverse=True)
        open_pending_notional = _btc15m_touch20_rules_open_pending_notional(ledger)
        ...
        if not candidates:
            ...
            return result
        if cap > 0 and remaining_cap <= Decimal("0"):
            result = {
                ...
                "gate": gate_summary,
                "funnel": dict(funnel),
                "selected": _btc15m_touch20_rules_selection_summary(candidates[0]),
                "open_pending_notional_dollars": _money_text(open_pending_notional),
        ...
        count_fp = _btc15m_touch20_rules_count_for_cap(remaining_cap, entry_side_price)
        if count_fp is None:
            result = {
                ...
                "frequency": freq,
                "asset_symbol": asset,
                "selected": _btc15m_touch20_rules_selection_summary(selected_item),
        ...
        ticket = TradeTicket(
            ...
            count_fp=count_fp,
            capital_bucket="btc15m_touch20_rules",
            note="btc15m_touch20_rules non-model entry",
        )
        client_order_id = _btc15m_touch20_rules_client_order_id(
            "entry",
            market_ticker=market.market_ticker,
            side=side_text,
            now=now,
        ...
        room = Room(
            name=f"BTC 15m touch20 rules {market.market_ticker}",
            market_ticker=market.market_ticker,
            kalshi_env=self.settings.kalshi_env,
            shadow_mode=False,
        )
        receipt = await self.base_execution_service.execute(
            ...
            ticket=ticket,
            client_order_id=client_order_id,
            fair_yes_dollars=_btc15m_touch20_rules_fair_yes_for_candidate(selected),
        )
        receipt_details = receipt.details if isinstance(receipt.details, dict) else {}
        ...
        async with self.session_factory() as session:
            ...
            if order_status not in {"shadow_skipped", "kill_switch_blocked", "inactive_color_skipped", "write_credentials_missing"}:
                ledger["positions"][client_order_id] = _btc15m_touch20_rules_entry_payload(
                    market=market,
                    candidate=selected,
                    bucket=selected_item.get("bucket") or {},
                ...
                await repo.set_checkpoint(
                    _btc15m_touch20_rules_ledger_stream(self.settings.kalshi_env, asset, freq),
                    None,
                    ledger,
                )
            await repo.log_ops_event(
    ...
    async def exit_once(self, *, frequency: str = "15m", asset_symbol: str = "BTC") -> dict[str, Any]:
        ...
        asset = normalize_asset_symbol(asset_symbol)
        now = datetime.now(UTC)
        stream_name = _btc15m_touch20_rules_ledger_stream(self.settings.kalshi_env, asset, freq)
        async with self.session_factory() as session:
            repo = PlatformRepository(session, kalshi_env=self.settings.kalshi_env)
            control = await repo.get_deployment_control(kalshi_env=self.settings.kalshi_env)
            checkpoint = await repo.get_checkpoint(stream_name)
            ...
            await session.commit()
        open_entries = _btc15m_touch20_rules_open_entries(ledger)
        if not open_entries:
            return {
                "status": "no_open_strategy_positions",
                "strategy": BTC15M_TOUCH20_RULES_STRATEGY,
                "kalshi_env": self.settings.kalshi_env,
                "frequency": freq,
                "asset_symbol": asset,
        ...
        for client_order_id, entry in open_entries:
                        ...
                        evaluated.append(
                            {
                                "client_order_id": client_order_id,
                                "market_ticker": market_ticker,
                                "status": "exit_retry_cooldown",
                                "next_exit_retry_at": retry_at.isoformat(),
                            }
                        )
                        continue
                except ValueError:
            ...
            if str(entry.get("status") or "") == "exit_submitted":
                submitted_at_raw = entry.get("exit_submitted_at")
                try:
            ...
            if snapshot is None:
                ...
                continue
            market = _market_from_snapshot(snapshot)
            row = _crypto_live_market_row(market, spot_rows=spot_rows, settings=self.settings)
            side = str(entry.get("side") or "")
            sell_yes_price = _btc15m_touch20_rules_sell_yes_price(snapshot, side)
            if sell_yes_price is None:
            ...
            entry_side_price = _decimal(entry.get("entry_side_price_dollars") or "0")
            net_profit_pct = _btc15m_touch20_rules_net_profit_pct(
                entry_side_price=entry_side_price,
                exit_side_price=_btc15m_touch20_rules_side_value_from_yes_price(sell_yes_price, side),
                count_fp=count_fp,
                fee_rate=Decimal(str(self.settings.kalshi_taker_fee_rate)),
            )
            if net_profit_pct is None:
                ...
                continue
            protection = _btc15m_touch20_rules_profit_protection_review(
                entry,
                row=row,
            ...
            trigger = None
            if net_profit_pct >= Decimal(str(self.settings.crypto_btc15m_touch20_take_profit_pct)):
                trigger = "take_profit"
            ...
            evaluated.append(
                {
                    ...
                    "market_ticker": market_ticker,
                    "status": "evaluated",
                    "net_profit_pct": str(net_profit_pct.quantize(Decimal("0.0001"))),
                    "trigger": trigger,
                    "profit_protection": protection["review"],
                }
            )
            if trigger is None:
                continue
            exit_client_order_id = _btc15m_touch20_rules_client_order_id(
                "exit",
                market_ticker=market_ticker,
                side=side,
                now=now,
            ...
            receipt = await self.base_execution_service.close_position(
                ...
                side=side,
                count_fp=count_fp,
                yes_price_dollars=sell_yes_price,
            ...
            realized_pnl = _btc15m_touch20_rules_realized_pnl(
                entry_side_price=entry_side_price,
                exit_side_price=_btc15m_touch20_rules_side_value_from_yes_price(sell_yes_price, side),
                count_fp=count_fp,
                fee_rate=Decimal(str(self.settings.kalshi_taker_fee_rate)),
            ...
            entry["exit_yes_price_dollars"] = _money_text(sell_yes_price)
            entry["exit_side_price_dollars"] = _money_text(_btc15m_touch20_rules_side_value_from_yes_price(sell_yes_price, side))
            entry["exit_order_status"] = status
            entry["exit_receipt"] = receipt.model_dump(mode="json")
            if status in {"filled", "executed"}:
                entry["status"] = "closed"
                entry["closed_at"] = now.isoformat()
                entry["realized_pnl_dollars"] = _money_text(realized_pnl)
                entry["net_profit_pct"] = str(net_profit_pct.quantize(Decimal("0.0001")))
            ...
            async with self.session_factory() as session:
                ...
                ledger["updated_at"] = datetime.now(UTC).isoformat()
                await repo.set_checkpoint(stream_name, None, ledger)
                await repo.log_ops_event(
...
def _btc15m_touch20_rules_ledger_stream(kalshi_env: str, asset_symbol: str, frequency: str) -> str:
    return f"{BTC15M_TOUCH20_RULES_STRATEGY}:{kalshi_env}:{normalize_asset_symbol(asset_symbol)}:{normalize_frequency(frequency) or frequency}"


def _btc15m_touch20_rules_ledger_payload(
    checkpoint: Any | None,
    *,
    kalshi_env: str,
    asset_symbol: str,
    ...
    return {
        "schema_version": "btc15m-touch20-rules-ledger-v1",
        "strategy": BTC15M_TOUCH20_RULES_STRATEGY,
        "kalshi_env": kalshi_env,
...
def _btc15m_touch20_rules_open_entries(ledger: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    for client_order_id, entry in positions.items():
        ...
        if not isinstance(entry, dict):
            continue
        if str(entry.get("status") or "") in {"open", "exit_submitted"}:
            entries.append((str(client_order_id), entry))
    return entries


def _btc15m_touch20_rules_open_pending_notional(ledger: dict[str, Any]) -> Decimal:
    positions = ledger.get("positions") if isinstance(ledger.get("positions"), dict) else {}
    total = Decimal("0")
    for client_order_id, entry in positions.items():
        if not str(client_order_id).startswith(f"{BTC15M_TOUCH20_RULES_ORDER_PREFIX}:"):
        ...
        if not isinstance(entry, dict):
            continue
        if str(entry.get("status") or "") not in {"entry_submitted", "open", "exit_submitted"}:
            continue
        total += _decimal(entry.get("entry_notional_dollars") or "0")
    return total.quantize(Decimal("0.0001"))


def _btc15m_touch20_rules_daily_realized_pnl(ledger: dict[str, Any], now: datetime) -> Decimal:
    positions = ledger.get("positions") if isinstance(ledger.get("positions"), dict) else {}
    total = Decimal("0")
    for client_order_id, entry in positions.items():
        if not str(client_order_id).startswith(f"{BTC15M_TOUCH20_RULES_ORDER_PREFIX}:"):
        ...
        if closed_at.astimezone(UTC).date() == now.astimezone(UTC).date():
            total += _decimal(entry.get("realized_pnl_dollars") or "0")
    return total.quantize(Decimal("0.0001"))


def _btc15m_touch20_rules_rank_key(item: dict[str, Any]) -> tuple[Any, ...]:
    ...
    bucket_pnl_per_candidate = _decimal(bucket.get("net_pnl") or "0") / Decimal(sample_count)
    return (
        bucket_pnl_per_candidate,
        float(bucket.get("touch_rate") or 0.0),
        _decimal(candidate.get("model_probability") or "0"),
        -int(candidate.get("spread_bps") or 0),
        int(candidate.get("time_to_close_seconds") or 0),
    )


def _btc15m_touch20_rules_selection_summary(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "target_exit_side_price_dollars": (candidate.get("touch_strategy") or {}).get("target_exit_side_price_dollars")
        if isinstance(candidate.get("touch_strategy"), dict)
        else None,
        "touch_probability": candidate.get("model_probability"),
        ...
        "touch_strategy": candidate.get("touch_strategy"),
    }


def _btc15m_touch20_rules_count_for_cap(remaining_cap: Decimal, entry_side_price: Decimal) -> Decimal | None:
...
def _btc15m_touch20_rules_client_order_id(
    action: str,
    *,
    market_ticker: str,
    side: str,
...
def _btc15m_touch20_rules_fair_yes_for_candidate(candidate: dict[str, Any]) -> Decimal:
    side = str(candidate.get("side") or "yes")
    probability = _decimal(candidate.get("model_probability") or "0.5000")
    return _clamp_price(probability if side == "yes" else Decimal("1.0000") - probability)


def _btc15m_touch20_rules_entry_payload(
    *,
    market: CryptoMarket,
    ...
    entry_fee = estimate_kalshi_taker_fee_dollars(
        price_dollars=entry_side_price,
        count=count_fp,
        fee_rate=fee_rate,
    )
    return {
        ...
        "close_time": (market.close_time or market.expected_expiration_time).isoformat()
        if (market.close_time or market.expected_expiration_time) is not None
        else None,
        "bucket_key": candidate.get("bucket_key"),
        "bucket": bucket,
        "candidate": _btc15m_touch20_rules_selection_summary({"market": market, "candidate": candidate, "bucket": bucket}),
        "target_exit_side_price_dollars": (candidate.get("touch_strategy") or {}).get("target_exit_side_price_dollars")
        if isinstance(candidate.get("touch_strategy"), dict)
        else None,
...
def _btc15m_touch20_rules_side_value_from_yes_price(yes_price: Decimal, side: str) -> Decimal:
    return yes_price if str(side).lower() == "yes" else Decimal("1.0000") - yes_price


def _btc15m_touch20_rules_sell_yes_price(snapshot: CryptoMarketSnapshotRecord, side: str) -> Decimal | None:
    price = snapshot.yes_bid_dollars if str(side).lower() == "yes" else snapshot.yes_ask_dollars
    if price is None or price <= Decimal("0") or price >= Decimal("1"):
        return None
    return quantize_price(price)


def _btc15m_touch20_rules_realized_pnl(
    *,
    entry_side_price: Decimal,
...
def _btc15m_touch20_rules_net_profit_pct(
    *,
    entry_side_price: Decimal,
    exit_side_price: Decimal,
    count_fp: Decimal,
    ...
    if denominator <= Decimal("0"):
        return None
    return (_btc15m_touch20_rules_realized_pnl(
        entry_side_price=entry_side_price,
...
def _btc15m_touch20_rules_profit_protection_review(
    entry: dict[str, Any],
    *,
    row: dict[str, Any],
    ...
    armed = bool(entry.get("profit_protection_armed")) or net_profit_pct >= threshold
    max_profit = max(_decimal(entry.get("max_net_profit_pct") or "0"), net_profit_pct)
    side = str(entry.get("side") or "yes")
    side_multiplier = Decimal("1") if side == "yes" else Decimal("-1")
    spot_return_1 = _decimal(row.get("spot_return_1_pct") or "0")
    spot_return_3 = _decimal(row.get("spot_return_3_pct") or "0")
    adverse_spot = (side_multiplier * spot_return_1) < Decimal("0") and (side_multiplier * spot_return_3) < Decimal("0")
    adverse_quote = previous_profit is not None and net_profit_pct < previous_profit
    trigger = None
    if armed and net_profit_pct <= floor:
    ...
    history.append(
        {
            ...
            "net_profit_pct": str(net_profit_pct.quantize(Decimal("0.0001"))),
            "spot_return_1_pct": str(spot_return_1),
            "spot_return_3_pct": str(spot_return_3),
    ...
    return {
        ...
        "entry_updates": {
            ...
            "max_net_profit_pct": str(max_profit.quantize(Decimal("0.0001"))),
            "last_net_profit_pct": str(net_profit_pct.quantize(Decimal("0.0001"))),
...
def _crypto_touch_strategy_candidates(
    ...
    *,
    ...
    policy_context = _crypto_touch_policy_context(
        ...
        btc_15m_touch20_rules_policy=btc_15m_touch20_rules_policy,
    )
...
def _evaluate_crypto_touch20_rules_replay(
    rows: list[dict[str, Any]],
    *,
    settings: Settings,
    crypto_policy: RuntimeCryptoPolicy | None = None,
    ...
    scoped_rows = [
        ...
        for row in rows
        if _safe_normalize_asset_symbol(row.get("asset_symbol")) == asset
        and _crypto_frequency_for_row(row) == "15m"
        and row.get("label_yes") in {0, 1}
        and isinstance(row.get("decision_ts"), datetime)
        and isinstance(row.get("settlement_ts"), datetime)
    ]
    ...
    metrics = {
        ...
        "current_model_live_quality_candidate_count": len(trades),
        "live_quality_candidate_count": len(trades),
        "rules_live_quality_candidate_count": len(trades),
        "touch_count": touch_count,
        "touch_rate": _ratio(touch_count / len(trades)) if trades else 0.0,
        "settlement_hold_count": settlement_hold_count,
        "net_simulated_pl_dollars": float(net),
...
def _crypto_touch20_rules_replay_requirements(settings: Settings) -> dict[str, Any]:
    return {
...
def _crypto_touch20_rules_replay_gate_reasons(metrics: dict[str, Any], *, settings: Settings) -> list[str]:
    reasons: list[str] = []
    if not metrics:
    ...
    candidates = int(metrics.get("rules_live_quality_candidate_count", metrics.get("trade_candidate_count")) or 0)
    min_candidates = max(1, int(settings.crypto_btc15m_touch20_replay_min_candidates))
    net_pl = float(metrics.get("net_simulated_pl_dollars") or 0.0)
    min_net = float(settings.crypto_btc15m_touch20_replay_min_net_pnl_dollars)
    hard_cap_breaches = int(metrics.get("hard_cap_breaches") or 0)
    max_hard_cap = int(settings.crypto_btc15m_touch20_replay_max_hard_cap_breaches)
    touch_rate = float(metrics.get("touch_rate") or 0.0)
    min_touch_rate = float(settings.crypto_btc15m_touch20_replay_min_touch_rate)
    pnl_per_candidate = float(metrics.get("pnl_per_candidate_dollars") or (net_pl / candidates if candidates > 0 else 0.0))
    if candidates < min_candidates:
        reasons.append(f"BTC 15m touch20 rules replay candidate count {candidates} below minimum {min_candidates}.")
    if net_pl <= min_net:
        reasons.append(f"BTC 15m touch20 rules replay net P/L ${net_pl:.2f} does not clear required positive threshold.")
    if pnl_per_candidate < float(settings.crypto_btc15m_touch20_replay_min_pnl_per_candidate_dollars):
        reasons.append(
            f"BTC 15m touch20 rules replay P/L per candidate ${pnl_per_candidate:.4f} below minimum "
            f"${float(settings.crypto_btc15m_touch20_replay_min_pnl_per_candidate_dollars):.4f}."
        )
    if hard_cap_breaches > max_hard_cap:
        reasons.append(f"BTC 15m touch20 rules replay hard-cap breaches {hard_cap_breaches} exceed limit {max_hard_cap}.")
    if touch_rate < min_touch_rate:
        reasons.append(f"BTC 15m touch20 rules replay touch rate {touch_rate:.1%} below minimum {min_touch_rate:.1%}.")
    if not (metrics.get("allowed_bucket_keys") or []):
