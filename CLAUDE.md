...
**2026-06-19 — "SIGMA ONLY" live-trading scope (operator directive).** Models keep **training** but only **sigma** (`vol_normal_fair_value`) may deploy as a **live champion**: `CRYPTO_LIVE_CHAMPION_ALLOWLIST=vol_normal_fair_value` (config `crypto_live_champion_allowlist`, enforced in `_crypto_select_champion`). ML champions (`xgboost`/`lightgbm`/`sklearn_logistic`/`spot_distance_*`/`asset_time_calibration`) still train + report but never go live — they fall through to `market_mid_baseline` (stand down). All existing gates (profit / `crypto_model_max_brier_regression_vs_mid` / replay) still apply on top, so sigma only trades where it passes (offline join 2026-06-19: beats mid on BTC/ETH/HYPE; BNB/DOGE/SOL/XRP stand down). Takes effect as the continuous trainer re-selects each asset; verify champions flip to `vol_normal_fair_value`/`market_mid_baseline` via `crypto_model_artifacts`. The per-asset `model_type` statuses below predate this directive (they describe the unrestricted pool).
