...
- `CRYPTO_MODEL_MAX_BRIER_REGRESSION_VS_MID=0.16` (was 0.07): admits the mildly-miscalibrated profitable cluster (XRP/BNB/DOGE, all ≤15% regression, well-traded) while still excluding the badly-miscalibrated (ETH 20% / SOL 27% / sigma 78–90%). **The replay gate + decision-time fee/edge gates remain** as the downstream safety net, so this relaxes ONE conservative guard, not the whole stack. Neither field is tuner-managed (`crypto_model_max_brier_regression_vs_mid` is not in `TUNABLE_GATE_FIELDS`), so setting them via `.env` is allowed. **Revert** = set allowlist back to `vol_normal_fair_value` and ceiling back to `0.07`. Takes effect as the trainer re-selects each asset (cursor seeded to DOGE-first 2026-06-20); verify champions flip away from `market_mid_baseline` via `crypto_model_artifacts`. The per-asset `model_type` statuses below predate all of this.
