...
**2026-06-30 — STALE-SPOT root-cause fix (feature schema → `crypto-rich-v15`).** Root cause of the long-standing "crypto has no edge / sigma collapses to Brier ≈0.25" finding: training/replay feature rows were built in `mode=CRYPTO_SPOT_CONTEXT_HISTORICAL`, which **dropped `spot_tick` rows** and computed moneyness from the up-to-900s-stale 15m spot candle — **median ~458s stale on a 15m market** (avg ttc ~446s), making moneyness a coin flip → every spot feature (and the analytic vol fair value) collapsed to base rate. LIVE decisions (`mode=CRYPTO_SPOT_CONTEXT_LIVE`) already used the fresh ~20s ticks, so this was a silent **train/serve mismatch**: models fit on stale-spot features but served fresh-spot ones. Fix (`crypto/services.py` `_spot_context_for_decision` / `_prepare_spot_context_series`): historical mode now uses the freshest point-in-time spot (tick or candle), matching live; pre-tick-era rows (before dense tick collection started ~2026-06-05) transparently fall back to candles. **Validated without a retrain** (`scripts/diag_sigma_freshfix.py`): on 17,678 settled HYPE decisions, raw vol Brier **0.2515 (stale) → 0.1530 (fresh)** vs mid 0.1276 — i.e. fresh spot turns the sigma from noise back into a near-mid-quality signal. Schema bumped v14→**v15** so the trainer rebuilds all spot features fresh; champions must be re-verified after the rebuild (raw vol Brier should drop toward/below mid via `diag_sigma_collapse.py`/`diag_sigma_freshfix.py`). The v14 cross-venue-consensus basis was separately **refuted** as the sigma fix (cross-venue basis ~0.025% — too small to matter); v14 code is retained but was never the lever. Live app stayed on v13 throughout; do not deploy v15 live until the rebuild proves champions beat the mid on calibration.
...
- `kalshi.py` — REST (RSA-signed) + WebSocket client. **Order create/cancel use the V2 endpoints** (`POST`/`DELETE /portfolio/events/orders…`) as of 2026-07-02 — Kalshi 410s the v1 `POST /portfolio/orders` (`deprecated_v1_order_endpoint`; this silently broke ALL production orders while the bot was dormant, caught by the stale-quote pilot's first live order). Services still build v1-shaped payloads; `order_payload_v2()` translates at this single choke point ((buy,yes)→bid, (buy,no)→ask, (sell,yes)→ask, (sell,no)→bid; `count_fp`→`count`, `yes_price_dollars`→`price`, `subaccount`→int). **Pilot extended to HYPE/DOGE/ETH 2026-07-04; BTC DROPPED 2026-07-06 (kill rule: −$3.24 over 19 post-ceiling settles, 8W/11L, DOGE/HYPE positive — operator "drop btc for now")**; caps **scaled 2026-07-06** to 2 contracts / 2 open / $6 daily stop / 5-per-12h-UTC-window trade budget (10/day belt on top), per `docs/superpowers/specs/2026-07-06-profitability-push-2-design.md`; per-asset readout via `kalshi_bot.crypto.stale_pilot_readout` (kill rule −$2 @ ≥15 settles with positive peer). **XRP ADDED 2026-07-06** (`STALE_PILOT_ASSETS=BNB,HYPE,DOGE,ETH,XRP` in `scripts/stale_quote_pilot_watchdog.sh`) — 15m backtest was the missing check (XRP untested in the original 3-check validation; flagged by +$7.72 in the 1h paper shadow); `scripts/xrp_stale_backtest.py` (fresh-tick recompute + one-trade-per-market dedup, same detection constants as `stale_quote_pilot.py`) showed dedup net +3.74¢/ct @ dfair≥0.10 (n=746, 59% win) in the most-recent-2-weeks window, confirmed positive (+1.59¢/ct, n=1082) in a disjoint prior 2-week window — see "XRP verdict (2026-07-06)" in `docs/research/2026-07-02-stale-quote-taker-edge.md`.
...
The daemon's reconcile and heartbeat loops are stall-proof (2026-07-02): each iteration runs under `daemon_loop_stall_timeout_seconds` (default 300) and failures are logged-and-retried rather than killing the loop. Before this, a Postgres crash-recovery cycle could hang the reconcile loop on a dead connection for hours while the container stayed "healthy" — fills/settlement ingestion silently stopped (tell: `max(created_at)` in `fills` lags live trading). The Postgres "crashes" themselves were diagnosed + reproduced the same day: postmaster-as-PID-1 reaping an orphaned `pg_isready` healthcheck exec (exit 2 under host memory pressure) and mistaking it for a crashed backend → self-inflicted crash recovery. Fix is `init: true` on the postgres compose services — **only takes effect once the postgres container is recreated**. See `docs/operations/2026-07-02-daemon-reconcile-wedge.md`.
...
- **Exit management (stop-loss + take-profit) RE-ENABLED 2026-07-04 — deliberately covering ALL positions INCLUDING MANUAL** (`PRODUCTION_STOP_LOSS_ENABLED=true`, `PRODUCTION_CRYPTO_TAKE_PROFIT_ENABLED=true`; both daemon colors recreated). This REVERSES the 2026-06-18 operator rule ("bot must not manage manual trades"): after the 2026-07-03/04 manual session (−$56 fading BTC hourly brackets, 5W/12L), the operator explicitly chose "All positions incl. manual" + "both stop-loss and take-profit" with the tradeoffs stated (the bot WILL cut manual positions being averaged into; June data: loss-cuts 8/9 right, 15m profit-takes −$11 vs holding). The system still cannot distinguish bot from manual trades (`reconcile.py` ingests the whole account; `_crypto_strategy_code_for_ticker` attributes by ticker pattern — 15m tickers → CRYPTO_15M, hourly brackets/dailies → CRYPTO_1H, both in the enabled-strategy lists, so manual bracket/daily positions ARE covered). Config: 30% hard+trail stop (90s min hold), TP 50% on 15m / 30% on 1h. Revert = set both flags false in `.env` + recreate both daemons. **TP thresholds are prediction-scaled (2026-07-04):** where a fresh trained (non-baseline) champion exists for the position's market, the effective threshold = base × lerp(`crypto_take_profit_min_multiplier`=0.5, `crypto_take_profit_max_multiplier`=2.0, shrunk_remaining_edge/`crypto_take_profit_edge_ref_cents`=5¢) — exits sooner when the model says the move is done, holds while it still sees edge; inline forecasts throttled per market (`crypto_take_profit_prediction_refresh_seconds`=30). No model / baseline / stale / error → static base. Manual bracket/daily positions were unaffected only while 1h champions sat at baseline/stale; with 1h training re-enabled 2026-07-04, fresh non-baseline 1h champions WILL scale TP on hourly bracket positions too (within the operator's 07-04 all-positions authorization). Kill just this feature via `CRYPTO_TAKE_PROFIT_PREDICTION_SCALING_ENABLED=false`. Spec: `docs/superpowers/specs/2026-07-04-prediction-scaled-take-profit-design.md`.
