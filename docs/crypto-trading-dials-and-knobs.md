...
| `crypto_model_max_brier_regression_vs_mid` | `0.07` code default; **`0.16` set in `.env` 2026-06-20** | Deploy ceiling: rejects a profit-"deployable" champion whose Brier regresses more than this fraction vs the market-mid baseline (the miscalibrated-but-profitable artifact signature). **At 0.07 this vetoed EVERY profitable candidate (sigma + ML) — all are 12–27% worse than mid at 15m — so 0/7 deployed.** Raised to **0.16** (operator "use whatever you can model", 2026-06-20) to admit the mildly-miscalibrated profitable cluster (XRP +12% / BNB +14% / DOGE +15%, shrinkage-adjusted, well-traded) while still excluding the badly-miscalibrated (ETH +20% / SOL +27% / in-pipeline sigma +78–90%). The replay gate (profit + coverage; `calibration_better_than_mid=False` in the active pack ⇒ calibration is diagnostic there) and decision-time fee/edge gates remain downstream. Not in `TUNABLE_GATE_FIELDS`, so operator-settable via `.env`. Revert to `0.07` to restore strict calibration. Enforced in `_crypto_select_champion` (`_brier_ok`). |
