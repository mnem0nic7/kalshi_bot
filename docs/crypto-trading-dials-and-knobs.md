...
| `crypto_model_max_brier_regression_vs_mid` | `0.07` code default; **`0.30` set in `.env` 2026-06-20** | Deploy ceiling: rejects a profit-"deployable" champion whose Brier regresses more than this fraction vs the market-mid baseline (the miscalibrated-but-profitable artifact signature). **At 0.07 this vetoed EVERY profitable candidate (sigma + ML) — all 12–27% worse than mid at 15m — so 0/7 deployed.** Raised 0.07 → 0.16 → **0.30** (operator "use whatever you can model" then "get the rest live ready", 2026-06-20): at **0.30** ALL profitable ML candidates compete (XRP +12% / BNB +14% / DOGE +15% / ETH +20% / SOL +27%) and the **replay gate** (independent OOS profit+coverage; `calibration_better_than_mid=False` in the active pack ⇒ calibration diagnostic) is the final arbiter. Still excludes broken in-pipeline sigma (+78–90%). **Hard limit:** BTC + HYPE have no profitable candidate → stay baseline regardless. Decision-time fee/edge gates also remain. Not in `TUNABLE_GATE_FIELDS`, so operator-settable via `.env`. Revert to `0.07` for strict calibration. Enforced in `_crypto_select_champion` (`_brier_ok`). |
| `crypto_live_champion_allowlist` | `""` (no restriction) | CSV of `model_type`s permitted to become the **live/deployable champion**. Empty ⇒ any candidate may win. **2026-06-19 → 2026-06-20:** was set to `vol_normal_fair_value` for the **"sigma only"** directive, then **reset to empty 2026-06-20** (operator "use whatever strategy you can model, sigma or anything else") after a full sigma-only sweep deployed 0/7 and the in-pipeline `vol_normal_fair_value` was shown to collapse to Brier ≈0.25 on every asset (so it never deploys regardless; the offline spine-based sigma that beats mid is a different, non-pipeline code path). Empty now lets the best gate-passing model (in practice `sklearn_logistic`) win per asset. The profit / `crypto_model_max_brier_regression_vs_mid` / replay gates still apply on top. Operational trading-scope toggle, **not** a gate threshold (not in autonomous-gate-tuning authority). Enforced in `_crypto_select_champion`. |

| `CRYPTO_CALIBRATION_ISOTONIC_MIN_ROWS` | `1000` | Below this many calibration rows, the champion probability calibrator fits **Platt** (sigmoid) instead of isotonic — isotonic overfits on scarce data (Niculescu-Mizil & Caruana, ICML'05; deep-research 3-0). At/above it, isotonic. Per-asset single-holdout calibration routinely runs on a few hundred rows (OOF only attempts ≥2000), squarely the small-sample regime. Carryover of the weather finding (measured +20.0% vs +15.2% Brier on weather; same `_fit_probability_calibration`/`_apply_probability_calibration` path). Env-settable (not a `TUNABLE_GATE_FIELD`). Takes effect on the next trainer retrain after an image build. |
| `CRYPTO_CALIBRATION_OOF_SPLITS` | `3` | Out-of-fold calibration folds; 0/1 disables OOF → single-holdout. |
...
| `CRYPTO_MID_STACK_ENABLED` | `false` | Mid-anchored **convex blend** `p = w·model + (1−w)·mid`, `w∈[0,1]` (`crypto/mid_stacking.py`). Makes the forecast **never worse than the mid** (measured: raw model is +1%..+76% worse on Brier because it never sees the mid). `w` is fit on the champion's **out-of-sample walk-forward** predictions (`_crypto_model_candidate_report`) with a chronological **holdout gate** — `w>0` only if the blend beats the mid on the unseen tail, else `w=0` (pure mid). Attached to the champion (`report.champion_mid_blend`) and applied at inference (`_crypto_predictions_for_model` + live forecast) **only when w>0** so a profit-selected champion isn't neutered. **Default OFF.** Validation signal = per-asset `champion_mid_blend.w` (w>0 = beat mid OOS). NOTE: the earlier free-logistic design (`method="mid_stack"`) failed OOS (negative mid coef on collinear inputs) and is **abandoned** — `apply_mid_stack` ignores it → mid. Quality fix + readiness for when the cross-venue basis adds signal; NOT a profit switch. |
