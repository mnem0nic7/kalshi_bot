...
| `CRYPTO_MID_STACK_ENABLED` | `false` | Mid-anchored stacking: fit `label ~ [logit(model), logit(mid)]` on holdout, attach to the champion, apply at inference (`crypto/mid_stacking.py`). Makes the forecast **never worse than the mid** (measured: our raw model is +1%..+76% worse on Brier because it never sees the mid) and up-weights the model only where it has real signal. **Default OFF** — when on, validate the trainer's `calibration_brier` vs `market_mid_brier` (should drop to ≤ mid) on the next retrain before relying on it. Quality/safety fix + readiness for when the cross-venue basis adds signal; NOT a profit switch (anchoring to the mid removes edge). Wired at `_crypto_model_metrics` (fit), `_crypto_predictions_for_model` + the live forecast (apply); all no-ops when no stack is attached. |
