...
| `crypto_live_champion_allowlist` | `""` (no restriction) | CSV of `model_type`s permitted to become the **live/deployable champion**. Empty ⇒ any candidate may win (legacy). Set to `vol_normal_fair_value` for the operator **"sigma only"** directive (2026-06-19): every model still **trains + reports**, but ML champions (`xgboost_classifier`/`lightgbm_classifier`/`sklearn_logistic`/`spot_distance_*`/`asset_time_calibration`) never deploy live — they fall through to the `market_mid_baseline` (stand down). The profit / `crypto_model_max_brier_regression_vs_mid` / replay gates still apply **on top**, so sigma only goes live where it passes (per the 2026-06-19 offline join: BTC/ETH/HYPE; the others stand down). Operational trading-scope toggle, **not** a gate threshold, so it lives in settings (not the autonomous-gate-tuning authority). Enforced in `_crypto_select_champion`. |
