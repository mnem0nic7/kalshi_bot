...
For bounded manual catch-ups, the Kalshi REST limiter and candle fanout can be raised in the `CLI` environment. For example, `KALSHI_REST_RATE_LIMIT_PER_SECOND=80 KALSHI_REST_RATE_LIMIT_BURST=160 CRYPTO_HISTORY_CANDLE_CONCURRENCY=30` keeps one asset process at a time but fetches candlesticks concurrently. Full refresh catch-ups can also set `CRYPTO_COLLECT_SETTLED_CANDLES_ENABLED=false` because the next history bootstrap step captures candles. If recent settled and historical pages are confirmed newest-first, `CRYPTO_SETTLED_PAGINATION_STOP_AT_CUTOFF=true CRYPTO_HISTORICAL_PAGINATION_STOP_AT_CUTOFF=true` avoids walking old pages outside the requested window. Watch the per-asset log for `429 Too Many Requests`; observed exchange-side candlestick throttles may be lower than the account-wide read budget.
