...
- `autonomous-gates --domain all` is the deterministic gate-upgrade lane for current production. Weather now has scoped policy-book support on top of flat fallback thresholds: global/city/month/side entry-gate candidates can be staged in `AgentPackWeatherPolicyBook`, canaried on newly labeled live rows, and promoted only through active-pack assignment. Crypto policy remains separate and per asset.
