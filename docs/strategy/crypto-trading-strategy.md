...
| `*_1h_touch20_rules` | Disabled | Frequency-scoped 1-hour version of the non-model Touch20 rules path for BTC, HYPE, ETH, BNB, SOL, DOGE, and XRP. Production keeps the data-only 1h current collector separate, but the 1h touch worker, rules switch, and trading switch are disabled. | Not live eligible unless an operator explicitly re-enables the 1h worker, the per-asset rules and trading switches, and the matching gate and approval path. |
