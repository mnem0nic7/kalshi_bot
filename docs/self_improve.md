...
- `researcher`, `president`, `trader`, `risk_officer`, `ops_monitor`, and `memory_librarian` default to provider `none` in `builtin-deterministic-v1`.
- The provider router returns deterministic fallback payloads when `LLM_CALLS_ENABLED=false`; Gemini, hosted OpenAI, and local OpenAI-compatible providers are only constructed when that flag is explicitly true.
...
`autonomous-gates run` is the deterministic threshold authority for baseline gates. For weather, it builds an episode-level gate-learning recommendation from historical and forward-shadow bundles, validates the candidate through bundle-backed backtesting and modeling, and stages either global thresholds or scoped `AgentPackWeatherPolicyBook` entries inside an agent pack. Scoped runs use `--scope city --series-ticker ... --month ... --side ... --lane entry_gate`; unsupported scopes fall back to the flat global thresholds. For crypto, the same command reads crypto snapshots, candles, spot OHLC, replay/backtest artifacts, and labeled crypto outcomes, then stages per-asset crypto-policy changes only for assets with enough support and better walk-forward P/L without worse drawdown. The daemon runs `--domain all` after settlement reconciliation and on an active-color periodic heartbeat, so accepted weather or crypto recommendations do not wait indefinitely for another settlement event. `--dry-run` prints the candidate, validation report, and evidence fingerprint without mutating runtime state. `--bootstrap-promote-from-historical` is an explicit one-time startup path for weather only: it can promote a staged candidate from `builtin-deterministic-v1` if the full candidate pack, not just each individual gate, passes reserved historical holdout support, net-P/L, and drawdown rules. If the holdout has too few selected episodes or does not beat the current pack, the staged startup candidate is rejected so the next tuning run recomputes from clean evidence. `autonomous-gates status --domain all` reads the checkpoint, active color, active pack, active weather policy provenance, active crypto policy, fallback settings thresholds, threshold drift, and last canary/promotion result. This loop never rewrites `.env` or `config.py`; those remain fallback defaults, while promoted agent-pack policy is the live source of truth.
