...
`autonomous-gates run` is the settlement-driven path for baseline gate thresholds. It builds a gate-learning recommendation from historical and forward-shadow bundles, validates the candidate through bundle-backed backtesting and modeling, stages threshold changes inside an agent pack, and waits for settled out-of-sample canary rows before assigning the pack to the active color. `--dry-run` prints the candidate, validation report, and evidence fingerprint without mutating runtime state. `autonomous-gates status` reads the checkpoint and agent-pack rollout notes. This loop never rewrites `.env` or `config.py`; those remain fallback defaults, while promoted agent-pack thresholds are the live source of truth.
