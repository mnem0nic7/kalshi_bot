...
- **Windowed READ with warmup overlap (correctness-critical):** the READ phase loads raw snapshots/candles/spot from `effective_since = watermark - warmup_window` (not the full 180d), COMPUTE decision rows over that window, and **upsert the entire recomputed window** (not just `decision_ts > watermark`). Two reasons the overlap must be re-upserted, both setting the floor on `warmup_window`:
  1. **Feature look-back:** `_crypto_decision_rows` uses rolling windows / cross-asset returns / settlement-window features. A tail row's features look back before the watermark, so the warmup raw data must be present for it to compute *identically* to a full rebuild.
  2. **Late-arriving labels (the gap):** a row materialized before its market settled was persisted with `label_yes=None`. Its `decision_ts` is ≤ watermark, but its settlement only resolved since the last build. Re-upserting the overlap window (idempotent on `row_id`) refreshes those labels. Therefore `warmup_window ≥ max(longest feature look-back, longest market settlement delay + slack)`. For 15m/1h markets a few hours covers settlement comfortably; size to the larger of the two and verify via the parity test below.
...
1. **Parity (correctness-critical):** for a fixed dataset, incremental materialize (windowed read from `watermark - warmup_window` → recompute → upsert whole window) yields a feature store **identical** to a full rebuild — same `row_id`/`feature_hash` set, and **same `label_yes`** including for a row that was `None` at first build and settled afterward (proves late-label refresh). Add a test with (a) a row whose features look back across the watermark and (b) a row that settles between the two builds; assert incremental store == full-rebuild store.
